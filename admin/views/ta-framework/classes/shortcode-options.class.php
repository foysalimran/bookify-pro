 ...
 *
 */
if ( ! class_exists( 'BOP_Shortcoder' ) ) {
  class BOP_Shortcoder extends BOP_Abstract{
    ...
    public $args         = array(
      ...
      'class'            => '',
      'gutenberg'        => array(
        'title'          => 'BOP Shortcodes',
        'description'    => 'BOP Shortcode Block',
        'icon'           => 'screenoptions',
        'category'       => 'widgets',
        'keywords'       => array( 'shortcode', 'bop', 'insert' ),
    ...
    public function __construct( $key, $params = array() ) {
      ...
      add_action( 'wp_ajax_bop-get-shortcode-'. $this->unique, array( $this, 'get_shortcode' ) );

      if ( ! empty( $this->args['show_in_editor'] ) ) {

        $name = str_replace( '_', '-', sanitize_title( $this->unique ) );

        BOP::$shortcode_instances[] = wp_parse_args( array( 'name' => 'bop/'. $name, 'modal_id' => $this->unique ), $this->args );

        // elementor editor support
        if ( BOP::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'BOP', 'add_admin_enqueue_scripts' ) );
          add_action( 'elementor/editor/footer', array( 'BOP_Field_icon', 'add_footer_modal_icon' ) );
    ...
    public function add_footer_modal_shortcode() {
      ...
      if( ! wp_script_is( 'bookify-pro' ) ) {
        return;
      }

      $class        = ( $this->args['class'] ) ? ' '. esc_attr( $this->args['class'] ) : '';
      $has_select   = ( count( $this->pre_tabs ) > 1 ) ? true : false;
      $single_usage = ( ! $has_select ) ? ' bop-shortcode-single' : '';
      $hide_header  = ( ! $has_select ) ? ' hidden' : '';

    ?>
      <div id="bop-modal-<?php echo esc_attr( $this->unique ); ?>" class="wp-core-ui bop-modal bop-shortcode hidden<?php echo esc_attr( $single_usage . $class ); ?>" data-modal-id="<?php echo esc_attr( $this->unique ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'bop_shortcode_nonce' ) ); ?>">
          ...
          <div class="bop-modal-table-cell">
            ...
            <div class="bop-modal-inner">
              <div class="bop-modal-title">
                ...
                <div class="bop-modal-close"></div>
              </div>
              <?php

                echo '<div class="bop-modal-header'. esc_attr( $hide_header ) .'">';
                echo '<select>';
              ...
              <div class="bop-modal-insert-wrapper hidden"><a href="#" class="button button-primary bop-modal-insert"><?php echo $this->args['insert_title']; ?></a></div>
            </div>
          </div>
        </div>
      </div>
    ...
    public function get_shortcode() {
      ...
      if ( ! empty( $shortcode_key ) && wp_verify_nonce( $nonce, 'bop_shortcode_nonce' ) ) {
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $section['fields'] ) && $view !== 'repeater' ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              BOP::field( $field, $field_default, $shortcode, 'shortcode' );
          ...
          if ( ! empty( $repeatable_fields ) ) {
            ...
            $button_title    = ( ! empty( $section['button_title'] ) ) ? ' '. $section['button_title'] : esc_html__( 'Add New', 'bookify-pro' );
            $inner_shortcode = ( ! empty( $section['group_shortcode'] ) ) ? $section['group_shortcode'] : $shortcode;

            echo '<div class="bop--repeatable">';

              echo '<div class="bop--repeat-shortcode">';

                echo '<div class="bop-repeat-remove fas fa-times"></div>';
                ...
                foreach ( $repeatable_fields as $field ) {
                  ...
                  BOP::field( $field, $field_default, $inner_shortcode.'[0]', 'shortcode' );
            ...
            echo '<div class="bop--repeat-button-block"><a class="button bop--repeat-button" href="#"><i class="fas fa-plus-circle"></i> '. esc_html($button_title) .'</a></div>';
        ...
        echo '<div class="bop-field bop-error-text">'. esc_html__( 'Error: Invalid nonce verification.', 'bookify-pro' ) .'</div>';
      }
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'enqueue_block_editor_assets', array( 'BOP_Shortcoder', 'add_guteberg_blocks' ) );
      }

      if ( bop_wp_editor_api() ) {
        add_action( 'media_buttons', array( 'BOP_Shortcoder', 'add_media_buttons' ) );
      }
    ...
    public static function add_guteberg_blocks() {
      ...
      wp_enqueue_script( 'bop-gutenberg-block', BOP::include_plugin_url( 'assets/js/gutenberg.js' ), $depends );

      wp_localize_script( 'bop-gutenberg-block', 'bop_gutenberg_blocks', BOP::$shortcode_instances );

      foreach ( BOP::$shortcode_instances as $block ) {

        register_block_type( $block['name'], array(
          'editor_script' => 'bop-gutenberg-block',
        ) );
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( BOP::$shortcode_instances as $value ) {
        echo '<a href="#" class="button button-primary bop-shortcode-button" data-editor-id="'. esc_attr( $editor_id ) .'" data-modal-id="'. esc_attr( $value['modal_id'] ) .'">'. esc_html( $value['button_title'] ) .'</a>';
      }
