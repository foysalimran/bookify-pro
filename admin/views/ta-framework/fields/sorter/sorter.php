 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'BOP_Field_sorter' ) ) {
	class BOP_Field_sorter extends BOP_Fields {
		...
		public function render() {
			...
			$args = wp_parse_args(
				...
				array(
					'disabled'       => true,
					'enabled_title'  => esc_html__( 'Enabled', 'bookify-pro' ),
					'disabled_title' => esc_html__( 'Disabled', 'bookify-pro' ),
				)
			);
			...
			echo '<div class="bop-sorter" data-depend-id="' . esc_attr( $this->field['id'] ) . '"></div>';

			echo ( $args['disabled'] ) ? '<div class="bop-modules">' : '';

			echo ( ! empty( $args['enabled_title'] ) ) ? '<div class="bop-sorter-title">' . esc_attr( $args['enabled_title'] ) . '</div>' : '';
			echo '<ul class="bop-enabled">';
			...
			if ( $args['disabled'] ) {
				...
				echo '<div class="bop-modules">';
				echo ( ! empty( $args['disabled_title'] ) ) ? '<div class="bop-sorter-title">' . esc_attr( $args['disabled_title'] ) . '</div>' : '';
				echo '<ul class="bop-disabled">';
