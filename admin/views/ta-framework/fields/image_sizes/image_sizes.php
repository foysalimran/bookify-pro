...
if ( ! class_exists( 'BOP_Field_image_sizes' ) ) {
	/**
	 *
	 * Field: Image sizes.
	 *
	 ...
	 * @version 1.0.0
	 */
	class BOP_Field_image_sizes extends BOP_Fields {
		/**
		...
		public function render() {
			...
			$sizes = array_merge(
				$sizes,
				array(
					'full'   => esc_html__( 'Original uploaded image', 'bookify-pro' ),
					'custom' => esc_html__( 'Set custom size', 'bookify-pro' ),
				)
			);
			...
			if ( ! empty( $sizes ) ) {
				...
				$chosen_attr      = ( $args['chosen'] ) ? ' class="bop-chosen' . $chosen_rtl . '"' : '';
				...
				if ( ! empty( $sizes ) ) {
					...
					echo ! empty( $this->field['empty_message'] ) ? esc_html( $this->field['empty_message'] ) : esc_html__( 'No image sizes found.', 'bookify-pro' );
