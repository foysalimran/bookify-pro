...
if ( ! class_exists( 'BOP_Field_dimensions_advanced' ) ) {
	 ...
	 * @since 3.5
	 */
	class BOP_Field_dimensions_advanced extends BOP_Fields {
		...
		public function render() {
			...
			$is_unit = ( ! empty( $unit ) ) ? ' bop--is-unit' : '';
			...
			if ( ! empty( $args['all'] ) ) {
				...
				echo '<input type="number" name="' . esc_attr( $this->field_name( '[all]' ) ) . '" value="' . esc_attr( $value['all'] ) . '"' . wp_kses_post( $placeholder . $min ) . ' class="bop-input-number' . esc_attr( $is_unit ) . '" />';
				echo ( $unit ) ? '<span class="bop--label bop--unit">' . esc_html( $args['units'][0] ) . '</span>' : '';
				echo '</div>';
				...
				foreach ( $properties as $property ) {
					...
					echo '<input type="number" name="' . esc_attr( $this->field_name( '[' . $property . ']' ) ) . '" value="' . esc_attr( $value[ $property ] ) . '"' . wp_kses_post( $placeholder . $min ) . ' class="bop-input-number' . esc_attr( $is_unit ) . '" />';
					echo ( $unit ) ? '<span class="bop--label bop--unit">' . esc_html( $args['units'][0] ) . '</span>' : '';
					echo '</div>';
